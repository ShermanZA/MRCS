...
--FRIENDLIES:
	...
	self.friendliesSet = SET_GROUP:New():FilterCategoryGround():FilterCoalitions("blue"):FilterPrefixes(self.FriendlyTemplates):FilterOnce()
	...
	self.baddiesTable = {}
	self.baddiesSet = SET_GROUP:New():FilterCategoryGround():FilterCoalitions("red"):FilterPrefixes(self.BadGuyTemplates):FilterOnce()	
	self.baddiesSet:ForEachGroup(function (grp)
	table.insert(self.baddiesTable, grp:GetName())
		end )	
	self.Friendly = SPAWN:NewWithAlias(self.FriendlyTemplates[1],"FRD-GRP"):InitHeading(0,1):InitRandomizeTemplate(self.friendliesTable)
	self.Badgroup = SPAWN:NewWithAlias( self.BadGuyTemplates[1], 'NME-GRP' ):InitHeading(0,1):InitRandomizeTemplate(self.baddiesTable)
  ...
  if self.friendly_return_fire == true then
			...
			function GroupHoldFire(grp) 
				...
				MESSAGE:New(".... YOU ARE CLEARED HOT!", self.TICMessageShowTime - self.friendly_fire_time, ""):ToGroup(selectedGroup)
      ...
      if smokecolornum == 1 then self.smokecolor = "GREEN"
        spawngroup:Smoke(SMOKECOLOR.Green,55,1) smoke_sound = "greensmoke.ogg" end
      if smokecolornum == 2 then self.smokecolor = "RED"
        spawngroup:Smoke(SMOKECOLOR.Red,55,1) smoke_sound = "redsmoke.ogg" end
      if smokecolornum == 3 then self.smokecolor = "WHITE"
        spawngroup:Smoke(SMOKECOLOR.White,55,1) smoke_sound = "whitesmoke.ogg" end
      if smokecolornum == 4 then self.smokecolor = "ORANGE"
        spawngroup:Smoke(SMOKECOLOR.Orange,55,1) smoke_sound = "orangesmoke.ogg" end
      if smokecolornum == 5 then self.smokecolor = "BLUE"
        spawngroup:Smoke(SMOKECOLOR.Blue,55,1) smoke_sound = "bluesmoke.ogg" end
		
		if selectedGroup:IsAirPlane() then
			MESSAGE:New(selectedGroup:GetCallsign() ..", JTAC, ... STAND BY FOR NINE LINE... ", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("TYPE 3 CONTROL, BOMB ON TARGET. MISSILES FOLLOWED BY ROCKETS & GUNS.", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			...
			MESSAGE:New("4. " .. math.floor(enemyGroup:GetAltitude() * 3.28084) .. " Feet ASL", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			...
			MESSAGE:New("8. FROM THE " .. Direction .. " 300 to 500 METERS DANGER CLOSE!", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			...
			MESSAGE:New(selectedGroup:GetCallsign() ..", JTAC, ... STAND BY FOR FIVE LINE... ", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("1. TYPE 2 CONTROL, BOMB ON TARGET. MISSILES FOLLOWED BY ROCKETS & GUNS.", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("2. MY POSITION " .. friendlycoorstring .. " MARKED BY " .. self.smokecolor .. " SMOKE!", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("3. TARGET LOCATION: " .. Direction .. " 300 to 500 METERS!", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("4. ENEMY TROOPS AND VEHICLES IN THE OPEN, " .. self.distance_marking_text, self.TICMessageShowTime, ""):ToGroup(selectedGroup)
			MESSAGE:New("5. " .. shoulderDir .. " SHOULDER. PULL YOUR DISCRETION. DANGER CLOSE, FOXTROT WHISKEY!", self.TICMessageShowTime, ""):ToGroup(selectedGroup)
		end
	return self
	end)
                ...
                if EventData.IniGroup == samesameIniGroup then
                  ...
                  --casgroups:ForEachGroupPartlyInZone(zone,function (grp)  
					...
					if grp:IsPartlyOrCompletelyInZone(zone) == true then
						MESSAGE:New("... From JTAC: good effect on target!" ,CasSelf.TICMessageShowTime,""):ToGroup(EventData.IniGroup,13)
					end
				  end)      
  ...
  for _, _unitName in pairs(self.CasUnits) do
    if not self.MenusDone[_unitName] then 
      ...
      if _unit then
		...
		local checkInmenu = MENU_GROUP_COMMAND:New(_group, "Check-In", casMenu, self._OnStation,self , _group, _unit):Refresh()
		local checkOutMenu = MENU_GROUP_COMMAND:New(_group, "Check-Out", casMenu, self._OffStation,self , _group, _unit):Refresh()
          -- sub menus
